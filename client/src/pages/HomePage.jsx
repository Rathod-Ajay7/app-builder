...
function HomePage() {
  const { user, handlegenrate, generatingproject, project, loadingProject, logout, handledelet, loadProjects } = useAppContext();
    ...
    <div className="h-screen overflow-y-scroll text-white font-sans bg-[url('/bg-img.png')] bg-cover bg-center bg-no-repeat hidden-scrollbar">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 pb-20 mt-8 xl:mt-28">
        <div className="w-full max-w-2xl flex flex-col items-center">
            ...
            <div className="animate-marquee gap-3">
              {homeTags.map((tag, i) => (
                <button
                  ...
                  className="px-4 py-1.5 rounded-full text-sm text-white bg-white/10 border-white/25 hover:bg-white/20 transition cursor-pointer shrink-0 font-medium">
                  {tag}
                </button>
              ))}
            </div>
          ...
          {/*all projects*/}
          {!loadingProject && project.length > 0 && (
            <div className="mt-12 w-full">
              ...
              <div className="space-y-2 max-h-[80vh] overflow-y-auto pr-1">
                {project.map((p) => (
                  <div key={p._id} className="bg-white/5 border-white/10 rounded-lg px-4 py-3 flex item-center justify-between group hover:border-white/20 hover:bg-white/10 cursor-pointer backdrop-blur-md transition-all "
