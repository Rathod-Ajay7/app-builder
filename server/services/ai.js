...
import { filecodeschema, fileplanschema, revisionresultschema } from './aischema.js';
...
export async function generateProject(prompt, callbacks) {
    ...
    for (let round = 0; round <= maxRetryRounds; round++) {
        ...
        if (round > 0) {
            console.log(`[AI] Retry round ${round}/${maxRetryRounds} for ${pendingFiles.length} failed files: ${pendingFiles.map((f) => f.path).join(", ")}`);
        }
        ...
        const results = await pMap(
            ...
            async (file) => {
                try {
                    const startCb = callbacks?.onFileStart || callbacks?.onfilestart;
                    if (startCb) {
                        await startCb(file.path);
                    }

                    const singleResult = await generateSingleFile(file, plan.files, prompt, files);

                    const completeCb = callbacks?.onFileComplete || callbacks?.onfilecomplete;
                    if (completeCb) {
                        await completeCb(file.path, singleResult.code);
                    }
    ...
    return {
        files,
        description: plan.projectdescription || plan.projectname || "Generated project"
    };
}

export const genrateproject = generateProject;
    ...
    if (rawParsed && Array.isArray(rawParsed.operations)) {
        rawParsed.operations = rawParsed.operations.map((op) => {
            ...
            if (op.op === "create" && op.content) {
                const validation = validateAndFixCode(op.content, op.path, {});
                op.content = validation.code;
                if (validation.warnings.length > 0) {
                    console.log(`[Validator] Revision Create adjustments for ${op.path}:\n  - ${validation.warnings.join("\n  - ")}`);
                }
            } else if (op.op === "update" && op.replace) {
                const validation = validateAndFixCode(op.replace, op.path, {});
                op.replace = validation.code;
                if (validation.warnings.length > 0) {
...
export const reviseproject = reviseProject;